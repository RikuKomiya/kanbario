...
- **UX 反映 (バッジ + macOS 通知) も完了** — MVP としてはひと区切り
- 次は大きめの機能追加 (Settings UI、GRDB、etc.) か、実機ドッグフード運用フェーズ
...
### 🟡 UX 反映 (MVP の体感価値を上げる) ✅ 完了

- [x] **バッジ View** — `TaskCardView` に `activity: ClaudeActivity?` 引数を追加、`ActivityBadge` (Working / Your turn) をタイトル行右に描画。`TaskDetailView.terminalPane` ヘッダの "running" ハードコードも `activitiesByTaskID[task.id]` 連動に変更 (working / awaiting input / idle)。
- [x] **macOS 通知** — `UserNotifications` framework を `AppState.swift` に import、末尾に fileprivate `NotificationService`。`init` で `requestAuthorizationIfNeeded()`、`handleHook` の `case "notification"` で payload の `message` field を extract して `post(title: task.title, body: message)`。App Sandbox OFF なので entitlement 追加不要。
